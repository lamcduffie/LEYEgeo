...
setwd = ("T:/Landbirds/FOCAL_SPECIES/Lesser_Yellowlegs/Geolocator/LEYE_591")

list.files(path = "T:/Landbirds/FOCAL_SPECIES/Lesser_Yellowlegs/Geolocator/LEYE_591", pattern = NULL, all.files = FALSE,
           full.names = FALSE, recursive = FALSE,
           ignore.case = FALSE, include.dirs = FALSE, no.. = FALSE)

#For Intigeo geos: read the data into a dataframe called d.lux
d.lux <- readMTlux("T:/Landbirds/FOCAL_SPECIES/Lesser_Yellowlegs/Geolocator/LEYE_591/Z591_15Jun17_014524.lux") 
...
path <- thresholdPath(twilight = d.twl$Twilight, 
                      rise = d.twl$Rise, 
                      zenith=zenith0,
                      tol= c(0.16,0.12)) # Here is where you can mess with the tol values
...
May06 <- max(which(strptime(twl$Twilight,format = "%Y-%m-%d") == "2017-05-06"))

fixedx<- rep(F, nrow(x0))
fixedx[1:5] <- TRUE
# Fixed from May - end of file #
fixedx[May06:nrow(x0)] <- TRUE
...
list.files(path = "T:/Landbirds/FOCAL_SPECIES/Lesser_Yellowlegs/Geolocator/LEYE_591", pattern = NULL,
           full.names = TRUE)
BlueMarb <- raster::brick("T:/Landbirds/FOCAL_SPECIES/Lesser_Yellowlegs/Geolocator/LEYE_591/BlueMarble.tiff") 
...
plot(LEYE$movements[[5]],add = TRUE, col = bpy.colors(20), alpha = 0.6, legend = FALSE)
